...
public class ShortDescriptionParser : InlineParser
{
	...
	public override bool Match(InlineProcessor processor, ref StringSlice slice)
	{
		...
		string expectedString = "ShortDescription ";

		for (int i = 0; i < expectedString.Length; i++)
		{
			if (slice.NextChar() != expectedString[i])
			{
				return false;
			}
		}
		...
		if (current.IsWhiteSpaceOrZero() || current == ')' || current == '}')
		{
			...
			processor.Inline = new ShortDescription
			{
				Span =
							   {
								 Start = inlineStart,
				...
				Line = line,
				Column = column,
				Description = new StringSlice(slice.Text, start, end)
			};
